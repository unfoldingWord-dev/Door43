...
class helper_plugin_door43obscomparer extends DokuWiki_Plugin {
    public $source;
    public $target;
    public $ignore_ws;
    public $ignore_punct;
...
// Make sure that selected target language is in the array of languages or is 'ALL'
        ...
        if($this->source && $this->target) {
            ...
            $this->populateData($this->source);
            $this->getStats($this->source);

            if ($this->target == 'ALL') {
                foreach ($this->languages as $language) {
                    if ($language != $this->source) {
                        $this->populateData($language);
                        $this->getStats($language);
                        $this->collateWithSource($language);
                    }
                }
            } else {
                $this->populateData($this->target);
                $this->getStats($this->target);
                $this->collateWithSource($this->target);
            }
...
<input type="hidden" name="page" value="'.$_GET['page'].'">
    ...
    protected function populateData($language)
    {
    ...
    protected function getStats($language){
...
// Now get stats
        ...
        $langData['stats']['frameMedian'] = $this->calculateMedian($arr);
        $langData['stats']['frameAverage'] = $this->calculateAverage($arr);
        ...
        foreach($langData['chapters'] as $chapterIndex=>&$chapter){
            ...
            $chapter['stats']['frameMedian'] = $this->calculateMedian($arr);
            $chapter['stats']['frameAverage'] = $this->calculateAverage($arr);
    ...
    protected function calculateAverage($arr) {
        $count = count($arr); //total numbers in array
        $total = 0;
    ...
    protected function collateWithSource($language){
        ...
        $frameMedianRatio = $this->calculateMedian($sourceTargetRatios);
        $frameAverageRatio = $this->calculateAverage($sourceTargetRatios);
